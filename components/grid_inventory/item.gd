...
func rotate_90() -> void:
	...
	if rotated:
		...
		InventoryEvents.rotated.emit(0.0)
	else:
		tween.tween_property(%Sprite, "rotation_degrees", -90.0 , .1)
		InventoryEvents.rotated.emit(-90.0)
	rotated = !rotated
...
func _on_handle_button_down() -> void:
	dragging = true
	top_level = true
	InventoryEvents.drag_started.emit(self, item_sprite)
...
func _on_handle_button_up() -> void:
	...
	InventoryEvents.drag_stopped.emit(drop_loc)
